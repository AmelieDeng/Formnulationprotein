...
def generate_embeddings(fasta_path, output_dir):
    ...
    for seq_record in sequences:
        ...
        if os.path.exists(embedding_file):
            all_embedding = torch.load(embedding_file, weights_only = False)
            embedding = all_embedding['representations'][6]  # Load chain embedding
...
##0_conformation PPI18 too long
dataset = '0_solubility'
pdb_p = f'/media/ouyang/backup_disk/output/0data/pdb/gzpdb/{dataset}_gz'
esm_p = f'esm/{dataset}'
data_p = '/media/ouyang/backup_disk/output/0data/'   
