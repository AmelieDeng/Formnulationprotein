...
class GCN(torch.nn.Module):
    def __init__(self, **kwargs):
        ...
        #res blocks 
        self.reslayer1 = ResidualFCBlock(output_features_size + int(output_features_size / 2) + int(output_features_size / 4) + form_FC_dim3, 125, 32)
    ...
    def forward_once(self, data):
        ...
        x_res, edge_index, x_batch, x_pos,x_form_one, edge_attr = data['atoms'].embedding_features_per_residue, \
            data[self.edge_type].edge_index, \
            data['atoms'].batch, \
            data['atoms'].pos, data['atoms'].form_one, \
...
######formulation data                
        ...
        output = torch.cat([output_res, output_form, output_res_2, output_res_3], 1)
