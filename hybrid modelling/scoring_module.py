...
# 方差阈值法：移除低方差的特征
def variance_threshold_selection(data, variance_threshold=0.1):

    selector = VarianceThreshold(threshold=variance_threshold)
    X_high_variance = selector.fit_transform(data)
    
    # 获取剩余的特征名称
    X_high_variance_df = pd.DataFrame(X_high_variance, columns=data.columns[selector.get_support()])
    
    return X_high_variance_df


# 相关性分析：移除高度相关的特征
def correlation_threshold_selection(data, correlation_threshold=0.9):
    ...
    plt.title("Correlation Matrix after Variance Threshold")
    # plt.show()
    ...
    # 删除冗余特征
    X_reduced = data.drop(columns=to_remove)
    
    return X_reduced, to_remove


# 主函数：加载数据，应用方差阈值法和相关性分析
def preprocess_data(csv_file, variance_threshold=0.1, correlation_threshold=0.9):

    data = pd.read_csv(csv_file)
    
    # 第一步：方差阈值法
    data_high_variance = variance_threshold_selection(data, variance_threshold)
    
    # 第二步：相关性分析
    data_cleaned, removed_features = correlation_threshold_selection(data_high_variance, correlation_threshold)
    
    print(f"Removed columns based on correlation: {removed_features}")
    print(f"Remaining features after both methods: {data_cleaned.columns.tolist()}")
    
    return data_cleaned


def mutual_information_selection(data, target, threshold=0.1):

    mi_scores = mutual_info_classif(data, target)
    mi_scores_df = pd.DataFrame({'Feature': data.columns, 'Mutual Information': mi_scores})
    ...
    removed_features = mi_scores_df[mi_scores_df['Mutual Information'] < threshold]

    X_selected = data[selected_features['Feature'].tolist()]
